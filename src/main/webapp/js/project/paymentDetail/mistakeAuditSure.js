...
function setPass(status){
	var select = $('#mistakeAuditTable').datagrid("getSelected");
	var refundStatus = select.refund_status;
	if (refundStatus == '6' || refundStatus == '7') {
		$.messager.alert('错误提示', '此订单已完成退款!', 'error');
		return false;
	}
...
//					return false;  
	...
	console.log(params);
	$.messager.confirm('差错退款审核退款提示', text, function(r) {
		
		// 首先如果用户选择了数据，则获取选择的数据集合
		if (r) {
			var url = root+'/errorRefunds/errorRefundConform';
			var options = {
				url : url,
				data : params,
				callBackFun : function(data) {
...
function cancelOrder() {
	var selecteds = $('#mistakeAuditTable').datagrid('getSelections');
	if(selecteds.length==0){
		$.messager.alert('错误提示', '请选择记录!', 'error');
		return false;
	}
	var select = selecteds[0];
	var url = root+ '/errorRefunds/cancelOrder';
	var options = {
		url : url,
		data : {
			"ordercode":select.ordercode,
