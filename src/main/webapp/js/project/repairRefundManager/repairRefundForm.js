...
function ajaxTable() {
	...
	$('#repairApplyTable').datagrid({
		...
		columns : [ [ {
			...
			field : 'refundAmountsActual',
			title : '退款金额',
			align : 'center',
			formatter : baseFormater,
			width : 75
		}, {
			field : 'refundAmountsDate',
			title : '退款日期',
			align : 'center',
			formatter : baseFormater,
			width : 75
...
/** -------- 补退款报表导出 ------ */
function exportForm() {
	var data = $('#repairApplyTable').datagrid('getData');
	var paper = $('#repairApplyTable').datagrid('getPager').pagination("options");
	var params = serializeJson("conditionForm");
	if(!params){
		return false;
	}
	...
	if(parseInt(total) > exportSize){
		...
		$.messager.prompt('提示', '每页导出数据65000条，当前数据量超过65000条，请输入要导出的页数：', function(r){
			if (r){
				...
				params["end"] = end;
				//return false;
				openWindowWithJson(root +'/repairRefund/exportRepairRefundForm',params);
			}
		});
	}else{
		params["start"] = 1;
