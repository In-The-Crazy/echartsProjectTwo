...
function initPage() {
	...
	$('#refundsAudit').dialog('close')
	$('#refundsRefuse').dialog('close')
}
/** --------加载表格数据 ------ */
function ajaxTable() {
...
function optFormater(value, row, index) {
	...
	if(row.refundStatus == "1"){
		detail = '<a href="javascript:void(0);" onclick=getInfo(' + _id + ',"refund")>补退款审核</a>';
	}else{
...
function audit() {
	...
	if(selecteds[0].refundStatus != 1){
		$.messager.alert('错误提示', '请选择未审核的记录!', 'error');
		return false;
	}
	$('#refundsApplyForm').form('reset');
	$('#refundsAudit').dialog('open');
}

function refundsAuditPass() {
	var selectedRow = $('#repairApplyTable').datagrid('getSelections')[0];
	var params = serializeJson("refundsApplyForm");
	var add = {
		url : root + '/repairRefund/auditPass',
		type : 'POST',
		data : {
...
//				return false;
		...
		console.log(params);
		$.messager.confirm('补退款审核提示', text, function(r) {
			
			// 首先如果用户选择了数据，则获取选择的数据集合
			if (r) {
				var url =root + '/repairRefund/auditPass';
				var options = {
					url : url,
					data : params,
					callBackFun : function(data) {
		...
		$.messager.confirm('补退款审核提示', text, function(r) {
			...
			// 首先如果用户选择了数据，则获取选择的数据集合
			if (r) {
				var url =root + '/repairRefund/auditRefuse';
				var options = {
					url : url,
					data : params,
					callBackFun : function(data) {
...
function refundsAuditRefuse() {
	var selectedRow = $('#repairApplyTable').datagrid('getSelections')[0];
	var params = serializeJson("refundsRefuseForm");
	var add = {
		url : root + '/repairRefund/auditRefuse',
		type : 'POST',
		data : {
			refundId : selectedRow.refundId,
			auditRemark : params.auditRemark,
