...
function ajaxTree() {
	...
	//添加-类型
	var add = {
		url : root+'/business/queryCategoryid',
		data: {"webid":"3"},
		callBackFun : function(data) {
			...
			$('#add_categoryid').combobox({
				data : data.treeList,
				valueField:'id',
				textField:'text',
				width : 150,
				onChange : function(newValue, oldValue){
					$("#add_header_fieldset").hide();
				}
			});
			$('#edit_categoryid').combobox({
				data : data.treeList,
				valueField:'id',
				textField:'text',
				width : 150,
				onChange : function(newValue, oldValue){
					$("#edit_header_fieldset").hide();
				}
			});
		}
	}
		...
		var edit = {
			...
			callBackFun : function(data) {
				if (data.isSuccessOrfail == "SUCCESS") {
					...
					$('#edit_categoryid').combobox('select',data.obj.categoryid);
