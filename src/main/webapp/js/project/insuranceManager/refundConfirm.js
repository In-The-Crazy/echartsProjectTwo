...
function optFormater(value, row, index) {
	...
	if(row.status == "4"){
		...
		detail = detail +  '<a href="javascript:void(0);" onclick=refundUnderLine(' + _id + ')>线下退款确认</a> | '
	}
...
function refundCheck(insuraId) {
	$.messager.confirm('退款确认提示', '是否确认提交退款?', function(r) {
		if (r) {
			var options = {
				url : root + '/insurance/insuranceConfirm',// 请求的action路径
				data : {
					"insrId": insuraId,
					"insStatus": '5'
				},
...
function refundUnderLine(insuraId) {
	$.messager.confirm('退款确认提示', '是否确认提交退款?', function(r) {
		if (r) {
			var options = {
					url : root + '/insurance/insuranceConfirm',// 请求的action路径
