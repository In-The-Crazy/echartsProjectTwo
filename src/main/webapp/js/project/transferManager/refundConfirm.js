...
function initDatebox() {
	...
	var first = (new Date((new Date()).getTime()-(30*24*60*60*1000))).pattern("yyyy-MM-dd")
	$('#beginDate').datebox('setValue',first);
	$('#finishDate').datebox('setValue',today);
}
...
function dateBoxValidator() {
	$('#beginDate').datebox().datebox('calendar').calendar({
		validator: function(date){
			var end = $('#finishDate').datebox('getValue');
			var now = new Date();
			if(!isEmpty(end)){
				now = new Date(end);
			}
	...
	$('#finishDate').datebox().datebox('calendar').calendar({
		validator: function(date){
			var now = new Date();
			var d1 = new Date(now.getFullYear(), now.getMonth(), now.getDate());
			return date<=d1;
...
function validateForm() {
	...
	var edate = $('#finishDate').datebox('getValue');
	if (!isEmpty(sdate)) {
...
function optFormater(value, row, index) {
	...
	var detail = "";
	if(row.cstatus == "1"){
		detail = '<a href="javascript:void(0);" onclick=refundCheck(' + _id + ')>退款确认</a> | ';
	}
	logger = '<a href="javascript:void(0);" onclick=getInfo(' + _id + ')>审核记录</a>';
...
function metorderCodeFormater(value, row, index) {
	...
	var detail = '<a href="javascript:void(0);" onclick=getOrderInfo(' + _id + ',' + _orderDate + ',' + _secondaryorderid + ')>' + _text + '</a>';
	return detail;
};
function orderCodeFormater(value, row, index) {
	...
	var detail = '<a href="javascript:void(0);" onclick=getMetOrderInfo(' + index + ');>' + text + '</a>';
	return detail;
};
...
/** --------查看详情 ------ */
function getMetOrderInfo(index){
...
function getOrderInfo(id, orderDate,secondaryorderid) {
	// 打开订单详情页面
...
/** -------- 确认退款 ------ */
function refundCheck(id) {
	$.messager.confirm('退款确认提示', '是否确认提交退款?', function(r) {
		if (r) {
			var options = {
				url : 'refundPass.action',// 请求的action路径
				data : {
					'id' : id
				},
				callBackFun : function(data) {
