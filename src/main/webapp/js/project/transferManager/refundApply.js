...
function ajaxTree() {
	$("#airCode").textbox({
		...
		onChange : function(newValue, oldValue){
			$(this).textbox("setValue",$(this).textbox("getValue").toUpperCase());
		}
	});
}
...
function orderCodeFormater(value, row, index) {
	...
	var detail = '<a href="javascript:void(0);" onclick=getOrderInfo(' + index + ');>' + text + '</a>';
	return detail;
};
...
function applyRefundByhand() {
	var selecteds = $('#queryOrderTable').datagrid('getSelections');
	...
	$.messager.confirm('提交退票提示', '是否确认提交退票? ', function(r) {
		...
		if (r) {
			var url = 'applyRefundByhand.action';
			var options = {
				url : url,
				data : {
					"clearstatus" : rows.clearstatus,
					"id" : rows.id
				},
				callBackFun : function(data) {
					if (data.isSuccessOrfail == "SUCCESS") {
