...
function ajaxTable() {
	...
	var result;
	var options = {
		url : 'querySystemAccessLog.action',
		data : params,
		callBackFun : function(data) {
			...
			if(data.isSuccessOrfail='SUCCESS'){
				...
				$.each(result, function(i, value){
					names[i] = value.functionality;
					counts[i] = value.funCount;
				});
	...
	// 加载表格
	$('#functionalityUseTable').datagrid({
		url : 'querySystemAccessLog.action',
...
window.onresize = function() {
	...
	$('#functionalityUseTable').datagrid('resize');
...
/** -------- 导出 ------ */
function exportSystemAccessLog() {
	var data = $('#functionalityUseTable').datagrid('getData');
	var sdate = $('#startdate').datebox('getValue');
	var edate = $('#enddate').datebox('getValue');
	...
	if(count>0){
		openWindowWithPost('exportSystemAccessLog.action',keys,values);
	}else{
