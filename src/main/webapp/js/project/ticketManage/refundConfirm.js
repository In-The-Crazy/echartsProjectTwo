...
function optFormater(value, row, index) {
	...
	var detail = "";
	if (row.isIntegral == '1' && row.returnIntegralStatus == '0' && row.status == "4") {
		detail = '<a href="javascript:void(0);" onclick=getInfo(' + _id + ',"info")>查看详情</a>';
	}else if(row.status == "4" || row.status == "8"){
		detail = '<a href="javascript:void(0);" onclick=getInfo(' + _id + ',"refund")>退款确认</a>';
	}else{
		detail = '<a href="javascript:void(0);" onclick=getInfo(' + _id + ',"info")>查看详情</a>';
	}
...
function refundCheck() {
	...
	$.messager.confirm('退款确认提示', '是否确认提交退款?', function(r) {
		if (r) {
			var options = {
				url : root + '/refund/refundConform',// 请求的action路径
				data : params,
				callBackFun : function(data) {
					if (data.isSuccessOrfail == "SUCCESS") {
						$('#refundInfo').dialog('close');
...
function batchRefundCheck() {
	var flag = true;
	var selects = $("#refundCheckTable").datagrid('getSelections');
	if (selects.length == 0) {
	...
	$.each(selects,function(i,select){
		if(select.status != "4"){
			$.messager.alert('错误提示', '请选择已审核的记录!', 'error');
			flag = false;
			return false;
		}
		ids += select.id+",";
	...
	if (flag) {
		...
		$.messager.confirm('退款确认提示', '是否确认提交退款?', function(r) {
			if (r) {
				var options = {
					url : root + '/refund/batchRefundConform',// 请求的action路径
					data : params,
					callBackFun : function(data) {
						if (data.isSuccessOrfail == "SUCCESS") {
							$('#refundInfo').dialog('close');
