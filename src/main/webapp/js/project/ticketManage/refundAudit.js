...
function refundCheck() {
	$.messager.confirm('退票审核提示', '是否确认提交审核?', function(r) {
		if (r) {
			...
			var options = {
				url : root + '/refund/firstRefundPass',// 请求的action路径
				data : data,
				callBackFun : function(data) {
...
function endEditing() {// 结束编辑
	...
	if ($('#refundPayTable').datagrid('validateRow', editIndex)
			|| $('#refundInfoTable').datagrid('validateRow', editIndex)) {
