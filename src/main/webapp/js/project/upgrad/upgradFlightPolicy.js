...
function initDatebox() {
	$('#sellStartTimes').datebox({
		...
		onSelect : function(beginDate) {
			$('#sellStartTimee').datebox().datebox('calendar').calendar({
	...
	$('#sellEndTimes').datebox({
		...
		width : 250,
		onSelect : function(beginDate) {
			$('#sellEndTimee').datebox().datebox('calendar').calendar({
				validator : function(date) {
					return beginDate <= date;
				}
			});
	...
	$('#flightDateStarts').datebox({
		...
		width : 250,
		onSelect : function(beginDate) {
			$('#flightDateStarte').datebox().datebox('calendar').calendar({
				validator : function(date) {
					return beginDate <= date;
				}
			});
	...
	$('#flightDateEnds').datebox({
		...
		width : 250,
		onSelect : function(beginDate) {
			$('#flightDateEnde').datebox().datebox('calendar').calendar({
				validator : function(date) {
					return beginDate <= date;
				}
			});
	...
	$('#add_flightDateStart,#edit_flightDateStart').datebox({
		label : '航班起始日期：',
		labelWidth : 100,
		labelAlign : "right",
		editable : false,
		...
		width : 250,
		onSelect : function(beginDate) {
			$('#add_flightDateEnd').datebox().datebox('calendar').calendar({
				validator : function(date) {
					return beginDate <= date;
				}
			});
	...
	$('#add_flightDateEnd,#edit_flightDateEnd').datebox({
		label : '航班截止日期：',
		labelWidth : 100,
		labelAlign : "right",
		editable : false,
...
function initPage() {
	...
	$("#add_policyCode,#edit_policyCode").textbox({
		label: '规则批号：',
		labelWidth: 100,
		labelAlign: "right",
		required: true,
	...
	$("#add_sailFrom,#edit_sailFrom").textbox({
		label: '航段：',
		labelWidth: 100,
		labelAlign: "right",
		required: true,
	...
	$("#add_sailTo,#edit_sailTo").textbox({
		required: true,
		width: 70
	})
	$("#add_adtFare,#edit_adtFare").numberbox({
		label: '成人升舱价格：',
		labelWidth: 100,
		labelAlign: "right",
		required: true,
	...
	$("#add_chdFare,#edit_chdFare").numberbox({
		label: '儿童升舱价格：',
		labelWidth: 100,
		labelAlign: "right",
		required: false,
	...
	$("#add_infFare,#edit_infFare").numberbox({
		label: '婴儿升舱价格：',
		labelWidth: 100,
		labelAlign: "right",
		required: false,
...
function ajaxTable() {
	...
	$('#policyTable').datagrid({
		...
		rowStyler: function (index, row) {
            if(row.isPastDue=='0'){
                return 'background-color:#EEC900;';
            }
        },
		columns : [ [ {
			field : 'policyId',
...
function ajaxTree() {
	...
	$('#add_fareType,#edit_fareType').combobox({
		label : '运价类型：',
		labelWidth : 100,
		labelAlign : "right",
		data : [{
	...
	if(parseInt(total) > exportSize){
		...
		$.messager.prompt('提示', '每页导出数据65000条，当前数据量超过65000条，请输入要导出的页数：', function(r){
			if (r){
				...
				params["end"] = end;
				openWindowWithJson(root +'/upgradFlightPolicy/exportPolicy',params);
			}
		});
	}else{
		params["start"] = 1;
...
function importPolicyList() {
	...
	xhr.open("POST", root+'/upgradFlightPolicy/importPolicy');
...
function policyImportReset() {
	$('#policyImprotForm').form('reset');
}