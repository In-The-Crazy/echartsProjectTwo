...
function ajaxTable() {
	...
	var options = {
		...
		callBackFun : function(data) {
			...
			if(data.isSuccessOrfail='SUCCESS'){
				...
				var airlineCounts = new Array();
				$.each(result, function(i, value){
					airlines[i] = value.airline;
					airlineCounts[i] = value.airlineCount;
				});
	...
	// 加载表格
	$('#airlineSelectStatsTable').datagrid({
		url : 'queryAirlineSelect.action',
...
window.onresize = function() {
	...
	$('#airlineSelectStatsTable').datagrid('resize');
...
/** -------- 导出 ------ */
function exportAirlineSelect() {
	var data = $('#airlineSelectStatsTable').datagrid('getData');
	var sdate = $('#startdate').datebox('getValue');
	var edate = $('#enddate').datebox('getValue');
	...
	if(count>0){
		openWindowWithPost('exportAirlineSelect.action',keys,values);
	}else{
